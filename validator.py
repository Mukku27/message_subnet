...
class Validator:
    ...
    def run(self):
        ...
        while True:
            try:
                # Create a synapse with a "hi" message
                synapse = Dummy(message="hi")

                # Broadcast a query to all miners on the network.
                responses = self.dendrite.query(
                    axons=self.metagraph.axons, synapse=synapse, timeout=12
                )
                bt.logging.info(f"sending message: {synapse.message}")
                if responses:
                    responses = [
                        response.response
                ...
                for i, resp_i in enumerate(responses):
                    current_score = 1 if resp_i == "hi this miner" else 0
